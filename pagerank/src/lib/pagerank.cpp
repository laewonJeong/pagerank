...
void Pagerank::calc_pagerank_value(int start, int end, double x, double y){
    ...
    for(int i=start;i<end;i++){
        ...
        if(pagerank.num_of_server != 1){
            d = round(((tmp + (double)(x/pagerank.num_of_vertex))*df + (double)((1-df)/pagerank.num_of_vertex))*10000000)/10000000;
            value = to_string(d);
...
void Pagerank::run_pagerank(int iter){
    ...
    for(int step =0; step < iter ;step++){
        ...
        if(pagerank.diff < 0.0000001 || pagerank.diff == prev_diff){//fabs(pagerank.diff - prev_diff) <0.0000001){
            break;
        }
